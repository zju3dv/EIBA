...
class AlignedMatrix3x8f
{
  public:
...
#if 0
	inline void Set(const AlignedMatrix3x3f &M0, const AlignedMatrix3x3f &M1, const AlignedVector3f &M2)
	{
    ...
    {
        if (UT::VectorAssertEqual(&m00(), &M.m00(), 24, 0)) return true;
        if (verbose) {
            UT::PrintSeparator();
            Print(verbose > 1);
            UT::PrintSeparator();
...
{
  public:
    inline EigenMatrix3x8f() = default;
    ...
    inline bool AssertEqual(const LA::AlignedMatrix3x8f &M,
                            const int verbose = 1) const
    {
        return GetAlignedMatrix3x8f().AssertEqual(M, verbose);
    }
    inline bool AssertEqual(const EigenMatrix3x8f &e_M,
                            const int verbose = 1) const
    {
        return AssertEqual(e_M.GetAlignedMatrix3x8f(), verbose);
    }
};
#endif
#endif
